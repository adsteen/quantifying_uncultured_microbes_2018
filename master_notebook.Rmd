...
for(i in seq_along(unique(trans_preprocessed$Environment_KL))) {
  ...
  #print(curr.envt)
  process_rank_order_data_v2(curr_df, ome.type = "metatranscriptome", envt = curr.envt)
}
```
...
p_finebins <- ggplot(all_finebin, aes(x=numeric.bins, y=frac.seqs, colour = method)) + 
  geom_line() + 
  scale_x_reverse(name = "Similarity to closest cultured relative", 
                  breaks = c(100, 90, 80, 70), labels = c("100%", "90%", "80%", "70%")) + 
  scale_y_continuous(name = "Fraction of sequences (%)", labels = percent) + 
  ...
  theme(text = element_text(size = 8),
        ...
        legend.position = c(0.61, 0.93)) 

if(save.files) {
  ggsave("plots/similarity_in_fine_bins_ColorBrewer.pdf", p_finebins, height = 4, width = 3.5, units = "in", dpi = 300)
...
p_arc_bac_boxplot <- ggplot(df_to_plot, aes(x=envt.to.plot, y = PctN, colour = envt.to.plot)) +
#p_arc_bac_boxplot <- ggplot(df_to_plot, aes(x=envt.to.plot, y = PctN)) +
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position = position_jitter(width = 0.25), alpha = 0.1) + 
...
if(save.files) {
  ...
  ggsave("plots/arc_bac_boxplot.png", p_arc_bac_boxplot, height = 4, width = 7, units = "in", dpi = 300)
...
if(save.files) {
  write_csv(filtered_pct_sim_by_metagenome, file="results/arc_bac_boxplot.csv")
...
make_identity_plots <- function(df) {
  # p_bulk_identity <- ggplot(df, aes(x=envt.ordered, y = PctN, shape = method, colour = Environment_KL)) + 
    p_bulk_identity <- ggplot(df, aes(x=envt.ordered, y = PctN, shape = method)) + 
    geom_point() + 
    scale_x_discrete(name = "Environment") + 
    scale_y_continuous(name = "Fraction in Bin", labels = percent) + 
...
if(save.files) {
  ggsave("plots/similarity_by_method_BW.png", p_all, height = 4, width = 3.5, units = "in", dpi = 300)
